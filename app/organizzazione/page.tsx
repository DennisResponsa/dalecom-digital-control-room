...
export default function OrganizationPage() {
  ...
  return <main className={styles.page}>
    ...
    {view === "procedure" && <section className={catalog.catalogShell}>
      <div className={catalog.catalogIntro}><div><small>23 CODICI DI PROCEDURA · A–U2</small><h2>Procedure operative complete</h2></div><p>Seleziona una sigla per leggere flusso, frequenze, responsabilità, soglie, descrizioni e checklist del documento originale.</p></div>
      ...
      <article className={`${catalog.catalogDetail} ${catalog.procedureDetail}`}><header><span>PROCEDURA {selectedProcedure.code}</span><small>{selectedProcedure.cadence}</small></header><h2>{selectedProcedure.title}</h2><b>{selectedProcedure.owner}</b><ol>{selectedProcedure.steps.map((step) => <li key={step}>{step}</li>)}</ol><footer>Contenuto completo · pronto per checklist, alert e task nel gestionale 1C</footer></article></div>
    </section>}
