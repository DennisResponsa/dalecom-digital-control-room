...
export default function TrainingDigitization() {
  ...
  const schedule = async () => {
    const employee = safetyEmployees.find((item) => item.name === draftName);
    if (!employee) return;
    const enrollment = createTrainingEnrollment({ employeeName: employee.name, employeeRole: employee.role, employeeBranch: employee.branch, course: selectedCourse, date: courseDate, trainer });
    ...
    try {
      const response = await fetch("/api/safety/course-enrollment", {
        ...
        body: JSON.stringify({ employeeName: employee.name, employeeRole: employee.role, employeeBranch: employee.branch, courseCode: selectedCourse.code, courseDate, trainer }),
      });
    ...
    setNotice(transportConfigured
      ? `Fascicolo di ${employee.name} generato e inviato automaticamente a ${COURSE_CERTIFIER_EMAIL}.`
      : `Fascicolo di ${employee.name} generato senza compilazione manuale. Destinatario bloccato su ${COURSE_CERTIFIER_EMAIL}; il trasporto automatico deve essere collegato prima della produzione.`);
  };
  const sendEmailDraft = (enrollment: TrainingEnrollment) => {
    const subject = encodeURIComponent(`Dalecom · iscrizione ${enrollment.courseCode} · ${enrollment.employeeName}`);
    const body = encodeURIComponent(`Buongiorno,\n\ntrasmettiamo il fascicolo digitale già precompilato per:\nDipendente: ${enrollment.employeeName}\nCorso: ${enrollment.courseTitle}\nData: ${enrollment.date}\nDocente: ${enrollment.trainer}\n\nRiferimento: ${enrollment.id}\n\nDalecom HSE`);
  ...
  const documentPreview = (document: DigitalCourseDocument) => {
    ...
    const commonRows = [
      ["Azienda", "Dalecom S.r.l."], ["Dipendente", active.employeeName], ["Mansione", active.employeeRole],
      ["Sede", active.employeeBranch], ["Corso", active.courseTitle], ["Codice corso", active.courseCode],
    ...
    const specificRows: Record<string, string[][]> = {
      ...
      attendance: [["Partecipante", active.employeeName], ["Ingresso / uscita", "Da registrare automaticamente il giorno del corso"], ["Presenza", "In attesa dell’avvio del corso"]],
      ...
      minutes: [["Esito test", active.testScore === null ? "Da completare" : `${active.testScore}%`], ["Firma dipendente", active.employeeSigned ? "Acquisita" : "Da acquisire"], ["Firma docente", active.trainerSigned ? "Acquisita" : "Da acquisire"]],
  ...
  return <section className={styles.shell}>
    ...
    {view === "planner" && <div className={styles.planner}>
      <aside className={styles.employeeRail}>
        ...
        <div>{employees.map((employee) => <button key={employee.name} draggable onDragStart={() => setDraggedName(employee.name)} onDragEnd={() => setDraggedName(null)} onClick={() => setDraftName(employee.name)}>
      ...
      <section className={styles.scheduleBoard}>
        ...
        <div className={`${styles.dropzone} ${draggedName ? styles.dragging : ""}`} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); if (draggedName) setDraftName(draggedName); setDraggedName(null); }}>
          <span>2</span><div><b>Assegna il dipendente a “{selectedCourse.title}”</b><small>Trascina qui il nome. Nessun modulo da compilare.</small></div>
        </div>
        {draftName && <div className={styles.setup}>
          <header><div><small>DIPENDENTE SELEZIONATO</small><h3>{draftName}</h3></div><button onClick={() => setDraftName(null)}>×</button></header>
          ...
          <footer><div><small>ALLA CONFERMA</small><b>Generazione automatica fascicolo + preparazione invio a {COURSE_CERTIFIER_EMAIL}</b></div><button onClick={schedule}>Genera fascicolo</button></footer>
        </div>}
        <div className={styles.enrollmentList}>
          ...
          {enrollments.map((item) => <article key={item.id}><div className={styles.statusDot} /><div><small>{item.courseCode} · {item.date}</small><b>{item.employeeName}</b><span>{item.courseTitle}</span></div><em>{item.status}</em><div className={styles.rowActions}><button onClick={() => sendEmailDraft(item)}>Email certificatore</button><button onClick={() => openTablet(item.id)}>Apri tablet →</button></div></article>)}
        </div>
      </section>
    </div>}

    {view === "tablet" && active && <div className={styles.tabletShell}>
      <aside className={styles.tabletPerson}><div className={styles.bigAvatar}>{active.employeeName.split(" ").map((part) => part[0]).slice(0, 2).join("")}</div><small>CARTELLA DIGITALE</small><h3>{active.employeeName}</h3><p>{active.employeeRole}<br />Sede {active.employeeBranch}</p><hr /><b>{active.courseTitle}</b><span>{active.date} · {active.trainer}</span></aside>
